...
@section('header', 'Edit Link Layanan')
    ...
    <div class="col-md-6">
        <div class="card card-warning">
            <div class="card-header">
                <h3 class="card-title">Form Edit Link</h3>
            </div>
            <form action="{{ route('admin.links.update', $link->id) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="title" placeholder="Contoh: PPDB Online" value="{{ old('title', $link->title) }}" required>
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="url" class="form-control @error('url') is-invalid @enderror" id="url" placeholder="https://... atau /voting/login" value="{{ old('url', $link->url) }}" required>
                        @error('url')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select name="target" id="target" class="form-control">
                            <option value="_blank" {{ old('target', $link->target) == '_blank' ? 'selected' : '' }}>Tab Baru (_blank)</option>
                            <option value="_self" {{ old('target', $link->target) == '_self' ? 'selected' : '' }}>Tab Sama (_self)</option>
                        </select>
                    </div>
                </div>
